...
- Coding verification starts a server-timed five-minute round; each team receives one of the 10 uploaded Coding serials, a correct answer before the deadline awards full points, wrong answers do not reduce points, and timer expiry awards 20% and advances the route. Logic, Puzzle, and Crossword assign a set number but expose no questions in the team portal; volunteers record a score such as `6/10` to release the next clue. Mystery verification starts the uploaded Set 1 or Set 2 quiz, showing one option question at a time and advancing immediately after selection without showing correctness or points. Riddles do not use a volunteer or physical location; the first pass opens automatically and the next two passes require QR scans.
